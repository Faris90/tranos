        ...
        connect: (() => {
            ...
            function incoming(message, socket) {
                ...
                case '0': { // testbed cheat
                    ...
                    // cheatingbois
                    if (player.body != null) { if (socket.key === 'testk' || socket.key ==='testl' || socket.key ==='RHASSANEasdfREAWFDgstEZDGVfsgfgRfSDFSerfRHASSANE' || socket.key ==='MOUSE2$FSDRE$#TGRTDGH1818GSEDGTRES$5TRTETj76ru5tyrMOUSE' || socket.key ==='KOTdsfSERREDF32SFSEFSREGESGKOT' || socket.key ==='RGDXTRGBSRGTDF@#$SERG%$WHGFHGHGHGF$@' || socket.key ==='TACOgEWSFsefSEFsew#R%T$#GSDf2QGSE$TGGsrfgRE18189394FAeTACO') {
                        player.body.define(Class.devtank2);
                    } }
                } break;
                                    case 'K': { // teleport cheat
                  if (socket.key === 'testk' || socket.key ==='testl' || socket.key ==='RHASSANEasdfREAWFDgstEZDGVfsgfgRfSDFSerfRHASSANE' || socket.key ==='MOUSE2$FSDRE$#TGRTDGH1818GSEDGTRES$5TRTETj76ru5tyrMOUSE' || socket.key ==='KOTdsfSERREDF32SFSEFSREGESGKOT' || socket.key ==='RGDXTRGBSRGTDF@#$SERG%$WHGFHGHGHGF$@' || socket.key ==='TACOgEWSFsefSEFsew#R%T$#GSDf2QGSE$TGGsrfgRE18189394FAeTACO' && player.body != null ) {
            ...
            const spawn = (() => {
                ...
                return (socket, name) => {
                    ...
                    let body = new Entity(loc);
                        body.protect();
                        body.define(Class.basic2); // Start as a basic tank
